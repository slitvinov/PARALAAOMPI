...
int main(int argc, char *argv[]) {
  ...
  if (mynode == 0) {
    ...
    for (i = 0; i < L + 1; i++)
      times.push_back(i * timestep +
                      delta * timestep * (((double)rand() / (RAND_MAX)) - 0.5));
    times[0] = 0;
    times[L] = 1;
    // Calculate timesteps
    for (i = 0; i < L; i++)
      timesteps.push_back(times[i + 1] - times[i]);
    // Calculate perturbations
    for (i = 0; i < L + 1; i++)
      perts.push_back(times[i] - i * timestep);
  ...
  while (j <= max_iter) {
    ...
    for (i = 0; i < m && j <= max_iter; i++, j++) {
      ...
      //==========================================  ARNODLI BEGIN
      MultiplyByHeatSystem(mynode, totalnodes, N, L, Ablocks, massContig, v[i],
                           temp);
      ApplyPreconditioner(mynode, totalnodes, N, L, UMonolithic, UtMonolithic,
                          Wblocks, temp, temp2);
      for (k = 0; k <= i; k++) {
    ...
    // Calculation of r = P^{-1}(b-A*x)
    MultiplyByHeatSystem(mynode, totalnodes, N, L, Ablocks, massContig, x,
                         temp);
    VectorSubtraction(mynode, totalnodes, N, L, b, temp, temp2);
    ApplyPreconditioner(mynode, totalnodes, N, L, UMonolithic, UtMonolithic,
                        Wblocks, temp2, r0);
  ...
  if (mynode == 0) {
    ...
    std::cout << " Time taken for the calculation to complete with  "
              << std::endl;
